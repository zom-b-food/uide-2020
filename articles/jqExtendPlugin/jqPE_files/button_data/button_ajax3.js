...
(function () {
    var Track = {links: function () {
        ...
        for (var i = 0; i < urls.length; i++) {
            ...
            if (document.defaultView != undefined) {var color = document.defaultView.getComputedStyle(link, "").getPropertyValue("color")}
            else {var color = link.currentStyle["color"]}
            if (color == "rgb(0, 0, 0)" || color == "#000000") {match != "" ? match += "," + escape(urls[i]) : match = escape(urls[i])}
        }
    ...
    var Button = {disable: function () {
        ...
        if (document.getElementById("tweeted")) {retweet.innerHTML = '<span class="left"></span>' + document.getElementById("tweeted").value + '<span class="right"></span>'}
        else {retweet.innerHTML = '<span class="left"></span>tweeted<span class="right"></span>'}
        ...
        for (var i = 0; i < objects.length; i++) {
            ...
            for (j in classes) {if (classes[j] == classname) {return objects[i]}}
        }
        return false
        ...
        for (var i = 0; i < params.length; i++) {
            ...
            if (input) {
                value = input.value;
                if (value != "") {url += "&" + params[i] + "=" + encodeURIComponent(value)}
            }
        }
        return url
    }};
    ...
    }, erase: function (name) {this.create(name, "", -1)}, read: function (name) {
        try {
            var nameEQ = name + "=";
    ...
    var Event = {add: function (object, action, onSuccess) {
        if (object.addEventListener) {object.addEventListener(action, onSuccess, false)}
        else {if (object.attachEvent) {object.attachEvent("on" + action, onSucces)}}
    }};
    var Ajax = {request: function (url, params, onSuccess) {
        ...
        if (xmlhttp != null) {
            if (onSuccess != undefined) {xmlhttp.onreadystatechange = onSuccess}
    ...
    if (c.innerHTML != "?") {
        ...
        var tweetedcheck = function () {
            if (!Button.beenTweeted()) {setTimeout(tweetedcheck, 1000)}
            else {if (timer > 1) {if (c.innerHTML.indexOf("k") == -1) {c.innerHTML = parseInt(c.innerHTML) + 1}}}
        };
