...
if (window.CavalryLogger) { CavalryLogger.start_js(["PXjzW"]); }
...
__d("ErrorUtils", ["Env"], function (a, b, c, d, e, f) {
    ...
    function n(w, x) {
        ...
        for (var y = 0; y < h.length; y++)try {h[y](w);}
        catch (z) {}
    }
    ...
    function s(w) {return function () {return r(w, this, arguments);};}

    function t(w, x, y) {n({message: w, script: x, line: y}, true);}
...
__d("Arbiter", ["ErrorUtils", "copyProperties", "createArrayFrom", "hasArrayNature"], function (a, b, c, d, e, f) {
    ...
    h(k, {SUBSCRIBE_NEW: 'new', SUBSCRIBE_ALL: 'all', BEHAVIOR_EVENT: 'event', BEHAVIOR_PERSISTENT: 'persistent', BEHAVIOR_STATE: 'state', LIVEMESSAGE: 'livemessage', BOOTLOAD: 'bootload', FUNCTION_EXTENSION: 'function_ext', subscribe: function (m, n, o) {
        ...
        return new l(q, r);
    }, unsubscribe: function (m) {m.unsubscribe();}, inform: g.guard(function (m, n, o) {
        var p = j(m);
        m = i(m);
...
__d("ArbiterMixin", ["Arbiter"], function (a, b, c, d, e, f) {
    var g = b('Arbiter'), h = {_getArbiterInstance: function () {return this._arbiter || (this._arbiter = new g());}, inform: function (i, j, k) {return this._getArbiterInstance().inform(i, j, k);}, subscribe: function (i, j, k) {return this._getArbiterInstance().subscribe(i, j, k);}, unsubscribe: function (i) {this._getArbiterInstance().unsubscribe(i);}, registerCallback: function (i, j) {this._getArbiterInstance().registerCallback(i, j);}};
    e.exports = h;
});
...
__d("isEmpty", [], function (a, b, c, d, e, f) {
    function g(h) {
        if (h instanceof Array) {return h.length === 0;}
...
__d("CSSLoader", ["isEmpty"], function (a, b, c, d, e, f) {
    ...
    function r(t, u, v, w) {
        ...
        if (!y)var z = setInterval(function aa() {if (q())clearInterval(z);}, h, false);
    }
...
__d("Bootloader", ["function-extensions", "Arbiter", "CSSLoader", "Env", "createArrayFrom", "isEmpty"], function (a, b, c, d, e, f) {
    ...
    function t(x, y, z, aa) {
        ...
        if (x == 'js') {
            ...
            ca.onreadystatechange = function () {if (this.readyState in {loaded: 1, complete: 1})ba();};
            aa.appendChild(ca);
        }
        else if (x == 'css')h.loadStyleSheet(z, y, aa, ba, function () {ba();});
    }
    ...
    var w = {configurePage: function (x) {
        ...
        if (!x)return [];
        var y = [];
        for (var z = 0; z < x.length; ++z)if (typeof x[z] == 'string') {if (x[z] in p)y.push(p[x[z]]);}
        else y.push(x[z]);
        return y;
...
__d("ge", [], function (a, b, c, d, e, f) {
    function g(h) {return typeof h == 'string' ? document.getElementById(h) : h;}
...
__d("CSS", ["$"], function (a, b, c, d, e, f) {
    var g = b('$'), h = 'hidden_elem', i = {setClass: function (j, k) {
        ...
        j = g(j);
        if (k)if (j.classList) {j.classList.add(k);}
        else if (!i.hasClass(j, k))j.className = j.className + ' ' + k;
        return j;
    }, removeClass: function (j, k) {
        j = g(j);
        if (k)if (j.classList) {j.classList.remove(k);}
        else if (i.hasClass(j, k))j.className = j.className.replace(new RegExp('(^|\\s)' + k + '(?:\\s|$)', 'g'), '$1').replace(/\s+/g, ' ').replace(/^\s*|\s*$/g, '');
        return j;
    }, conditionClass: function (j, k, l) {return (l ? i.addClass : i.removeClass)(j, k);}, toggleClass: function (j, k) {return i.conditionClass(j, k, !i.hasClass(j, k));}, shown: function (j) {return !i.hasClass(j, h);}, hide: function (j) {return i.addClass(j, h);}, show: function (j) {return i.removeClass(j, h);}, toggle: function (j) {return i.toggleClass(j, h);}, conditionShow: function (j, k) {return i.conditionClass(j, h, !k);}};
    e.exports = i;
});
__d("BlueBarController", ["Bootloader", "CSS"], function (a, b, c, d, e, f) {
    ...
    f.init = function (i) {
        if ('getBoundingClientRect' in i) {
            ...
            j();
            g.loadModules(['event-extensions'], function () {Event.listen(window, 'scroll', j);});
        }
    };
});
...
__d("Run", ["Arbiter", "OnloadEvent"], function (a, b, c, d, e, f) {
    ...
    function k() {return !window.loading_page_chrome;}

    function l(x) {
        var y = a.OnloadHooks;
        if (window.loaded && y) {y.runHook(x, 'onlateloadhooks');}
        else q('onloadhooks', x);
    }

    function m(x) {
        var y = a.OnloadHooks;
        if (window.afterloaded && y) {setTimeout(function () {y.runHook(x, 'onlateafterloadhooks');}, 0);}
    ...
    function o(x) {
        if (!window.onunload)window.onunload = function () {g.inform(h.ONUNLOAD, true, i);};
        q('onunloadhooks', x);
    }

    function p(x) {q('onleavehooks', x);}

    function q(x, y) {window[x] = (window[x] || []).concat(y);}

    function r(x) {window[x] = [];}

    function s() {g.inform(h.ONLOAD_DOMCONTENT, true, i);}
...
__d("legacy:onload", ["Run", "OnloadEvent"], function (a, b, c, d) {
    ...
    a.onloadRegister = function () {return e.onLoad.apply(this, arguments);};
    a.onafterloadRegister_DEPRECATED = e.onAfterLoad;
    a.onafterloadRegister = function () {return e.onAfterLoad.apply(this, arguments);};
...
__d("wait_for_load", ["Bootloader", "Run"], function (a, b, c, d, e, f) {
    ...
    function i(l, m) {return window.loaded && m.call(l);}
        ...
        switch ((m || event).type) {
            ...
            case 'click':
                ...
                h.onAfterLoad(function () {
                    o.cursor = p.cursor = '';
                    if (l.tagName.toLowerCase() == 'a') {if (false !== n() && l.href)window.location.href = l.href;}
...
__d("Intl", [], function (a, b, c, d, e, f) {
    ...
    function i(j) {
        if (g) {
            ...
            for (var r = 0; r < k.length; r++) {
                ...
                if (l[r] == 'javascript') {j.replace(s, function (t) {return t.slice(1).toLowerCase();});}
    ...
    e.exports = {endsInPunct: h, applyPhonologicalRules: i, setPhonologicalRules: function (j) {g = j;}};
});
...
__d("array-extensions", [], function (a, b, c, d, e, f) {
    Array.prototype.contains = function (g) {return this.indexOf(g) != -1;};
...
__d("JSCC", ["isEmpty"], function (a, b, c, d, e, f) {
    ...
    var l = {get: function (m) {
        ...
        return h[m]();
    }, init: function (m) {k(m);}, initForPagelet: function (m, n) {k(n, m);}, clearForPagelet: function (m) {
        if (i[m]) {
...
__d("ServerJS", ["function-extensions", "ErrorUtils", "copyProperties"], function (a, b, c, d, e, f) {
    ...
    h(i.prototype, {handle: g.guard(function (o) {
    ...
    }, _handleMarkup: function (o, p, q) {define(o, ['HTML'], function (r) {return r.replaceJSONWrapper(p).getRootNode();}, 0, null, q);}, _handleElement: function (o, p, q, r) {
        var s = [], t = 0;
        if (r) {
            s.push(r);
            t = 1;
        ...
        if (Array.isArray(q)) {for (r = 0; r < q.length; r++)this._replaceTransportMarkers(q, r);}
        else if (q && typeof q == 'object')if (q.__m) {o[p] = b(q.__m);}
        else if (q.__e) {o[p] = k(q.__e);}
        else if (q.__rel) {o[p] = this._relativeTo;}
    ...
    function l(o, p, q) {for (var r = 0; r < o.length; r++)p.apply(q, [].concat(o[r]));}
...
__d("invokeCallbacks", ["ErrorUtils"], function (a, b, c, d, e, f) {
    ...
    function h(i, j) {if (i)for (var k = 0; k < i.length; k++)g.applyWithGuard(new Function(i[k]), j);}
...
__d("BigPipe", ["Arbiter", "Bootloader", "Env", "ErrorUtils", "JSCC", "OnloadEvent", "Run", "ServerJS", "$", "copyProperties", "ge", "hasArrayNature", "invokeCallbacks"], function (a, b, c, d, e, f) {
    ...
    function w(ba) {return (!ba || r(ba) && ba.length === 0) ? {} : ba;}
    ...
    function y(ba) {
        if (!ba.firstChild) {
            h.loadModules(['ErrorSignal'], function (da) {da.sendErrorSignal('bigpipe', 'Pagelet markup container is empty.');});
            return null;
        }
    ...
    function aa(ba) {
        ...
        p(this, ba);
        if (this.automatic) {this._relevant_instance = aa._current_instance;}
    ...
    p(aa.prototype, {
        _displayPageletHandler: function (ba) {
            if (this.displayCallback) {this.displayCallback(this._displayPagelet.bind(this, ba));}
            ...
            h.loadResources(ba.js || [], function () {
                ...
                }.bind(this), da = function () {this._isRelevantPagelet(ba) && s(ba.onafterload);}.bind(this);
            ...
            if (da)if (ba.cache_hit) {ba = da.loadFromCache(ba, this.arbiter);}
            ...
            this._currentPhase++;
            if (t) {setTimeout(ca, 20);}
            else ca();
        }, _isRelevant: function () {return this == aa._current_instance || (this.automatic && this._relevant_instance == aa._current_instance) || this.jsNonBlock || this.forceFinish;}, _isRelevantPagelet: function (ba) {
...
__d("Class", ["function-extensions", "Arbiter", "copyProperties", "createArrayFrom"], function (a, b, c, d, e, f) {
    ...
    function n() {
        for (var t in j)if (!!a[t]) {
            delete j[t];
            if (!a[t].__class_extending) {g.inform(g.FUNCTION_EXTENSION + '/' + t, true, g.BEHAVIOR_STATE);}
...
__d("goURI", [], function (a, b, c, d, e, f) {
    function g(h, i, j) {
        h = h.toString();
        if (!i && a.PageTransitions && PageTransitions.isInitialized()) {PageTransitions.go(h, j);}
        else if (window.location.href == h) {window.location.reload();}
...
__d("InitialJSLoader", ["Arbiter", "Bootloader", "OnloadEvent"], function (a, b, c, d, e, f) {
    var g = b('Arbiter'), h = b('Bootloader'), i = b('OnloadEvent'), j = {INITIAL_JS_READY: 'BOOTLOAD/JSREADY', loadOnDOMContentReady: function (k, l) {
        g.subscribe(i.ONLOAD_DOMCONTENT_CALLBACK, function () {
            function m() {h.loadResources(k, function () {g.inform(j.INITIAL_JS_READY, true, g.BEHAVIOR_STATE);});}
...
__d("legacy:object-core-utils", ["copyProperties", "isEmpty"], function (a, b, c, d) {
    ...
    function f(g, h) {return e(window[g] || (window[g] = {}), h);}
...
__d("BanzaiCommon", ["BanzaiConfig"], function (a, b, c, d, e, f) {
    ...
    var u = a.sessionStorage ? {store: function w() {a.sessionStorage.setItem('Banzai', JSON.stringify(k));}, restore: function w() {
        if ('Banzai' in a.sessionStorage) {
            k = [].concat(JSON.parse(a.sessionStorage.getItem('Banzai')));
            l = [].concat(k);
            a.sessionStorage.removeItem('Banzai');
        }
    }} : {store: function () {}, restore: function () {}}, v = {SEND: 'Banzai:SEND', OK: 'Banzai:OK', ERROR: 'Banzai:ERROR', SHUTDOWN: 'Banzai:SHUTDOWN', VITAL: h, SEND_TIMEOUT: 15000, isEnabled: function (w) {return g.gks && g.gks[w];}, post: r, setAdapter: t, subscribe: function (w, x) {j.subscribe(w, x);}};
    p();
    u.restore();
    q(i.delay);
    e.exports = v;
...
__d("QueryString", [], function (a, b, c, d, e, f) {
    var g = {encode: function (h) {
        ...
        return i;
    }, appendToUrl: function (h, i) {return h + (~h.indexOf('?') ? '&' : '?') + (typeof i === 'string' ? i : g.encode(i));}};
    e.exports = g;
});
__d("XHR", ["Env"], function (a, b, c, d, e, f) {
    var g = b('Env'), h = {create: function () {
        try {return a.XMLHttpRequest ? new a.XMLHttpRequest() : new ActiveXObject("MSXML2.XMLHTTP.3.0");}
        catch (i) {}
...
__d("BanzaiDesktop", ["Arbiter", "BanzaiCommon", "Miny", "QueryString", "Run", "XHR"], function (a, b, c, d, e, f) {
    var g = b('Arbiter'), h = b('BanzaiCommon'), i = b('Miny'), j = b('QueryString'), k = b('Run'), l = b('XHR'), m = null, n = new g(), o = '/ajax/banzai/banzai.php', p = {inform: function q(r) {n.inform(r);}, subscribe: function (q, r) {n.subscribe(q, r);}, cleanup: function q() {
        if (m && m.readyState < 4)m.abort();
        if (m) {
            delete m.onreadystatechange;
            m = null;
        }
    }, isSending: function q() {return !!m;}, send: function q(r) {
        var s = 'POST';
        m = l.create();
        m.open(s, o, true);
        ...
        m.send(j.encode(t));
    }, onUnload: function q(r) {k.onUnload(r);}};
    h.setAdapter(p);
    e.exports = h;
});
__d("userAction", ["Arbiter", "BanzaiDesktop", "copyProperties"], function (a, b, c, d, e, f) {
    ...
    i(n.prototype, {_log: function (u, v) {
    ...
    }, _get_action_type: function () {return (this._primary._action_type || this._fallback._action_type || this._default_action_type);}, _get_ua_id: function () {return (this._primary._ua_id || this._fallback._ua_id || this._default_ua_id);}, _log_uai: function () {
        ...
        if (!this._uai_logged) {
            ...
            if (w === undefined) {this._log_uai();}
            else if (w === false) {this._uai_logged = true;}
            else {
                var x = this;
                w = w || 0;
                this._uai_timeout = setTimeout(function () {x._log_uai.apply(x);}, w);
            }
        }
        return this;
    ...
    s.setUATypeConfig = function (u) {i(l, u);};
    s.setCustomSampleConfig = function (u) {i(m, u);};
    s.getCurrentUECount = function () {return p;};
...
__d("Primer", ["function-extensions", "Bootloader", "CSS", "ErrorUtils", "Parent", "clickRefAction", "trackReferrer", "userAction"], function (a, b, c, d, e, f) {
    ...
    function q(t, u) {
        ...
        do {
            ...
            JSON.parse(v).forEach(function (w) {
                var x = t;
                g.loadModules.call(g, [w[0]], function (y) {y[w[1]](x);});
            });
    ...
    p.onclick = i.guard(function (t) {
        ...
        switch (ca) {
            case 'dialog':
            case 'dialog-post':
                g.loadModules(['AsyncDialog'], function (da) {da.bootstrap(y, v, ca);});
                break;
            case 'async':
            case 'async-post':
                g.loadModules(['AsyncRequest'], function (da) {da.bootstrap(y, v);});
                break;
            case 'theater':
                g.loadModules(['PhotoSnowlift'], function (da) {da.bootstrap(y, v);});
                break;
            case 'toggle':
                h.toggleClass(v.parentNode, 'openToggler');
                g.loadModules(['Toggler'], function (da) {da.bootstrap(v);});
                break;
            default:
                return u;
        }
    ...
    p.onsubmit = i.guard(function (t) {
        ...
        if (u && u.nodeName == 'FORM' && u.getAttribute('rel') == 'async') {
            ...
            var w = n;
            g.loadModules(['Form'], function (x) {x.bootstrap(u, w);});
            return false;
        }
    });
    var r = null, s = i.guard(function (t, u) {
        ...
        switch (v.getAttribute('data-hover')) {
            case 'tooltip':
                g.loadModules(['Tooltip'], function (w) {w.process(v, r);});
                break;
        }
    });
    p.onmouseover = s.curry('mouseover');
    if (p.addEventListener) {p.addEventListener('focus', s.curry('focus'), true);}
...
__d("CommentPrelude", ["CSS", "Parent", "clickRefAction", "userAction"], function (a, b, c, d, e, f) {
    ...
    function l(o, p) {
        ...
        if (window.ScrollAwareDOM) {window.ScrollAwareDOM.monitor(q, r);}
        else r();
    ...
    function m(o) {
        ...
        if (window.ScrollAwareDOM) {window.ScrollAwareDOM.monitor(o, p);}
        else p();
    }
