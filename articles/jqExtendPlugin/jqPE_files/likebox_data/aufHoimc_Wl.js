...
if (window.CavalryLogger) { CavalryLogger.start_js(["+USDd"]); }

function fbpage_set_fan_status(a, b, c, d, e, f, g) {
    e = e ? e : function (j) {_fbpage_show_change_status_feedback(a, j.getPayload());};
...
function fbpage_set_favorite_status(a, b, c) {
    var d = function () {_fbpage_show_change_status_feedback(a, this.getUserData());}, e = {fbpage_id: b, add: c}, f = new AsyncRequest().setMethod('POST').setURI('/ajax/pages/favorite_status.php').setData(e);
    new Dialog().setAsync(f).setCloseHandler(d).show();
    return false;
}
function _fbpage_show_change_status_feedback(a, b) {
    if (!b || !a)return;
    if (b.reload) {fbpage_reload_on_fan_status_changed(b.preserve_tab);}
...
function fbpage_redraw_on_fan_status_changed(a, b) {
    ...
    var d = function () {if (data.can_repeat_action)c.parentNode.replaceChild(a, c);};
...
var ExternalNodeConnectWidget = function (a) {
    ...
    if (h.length == 1) {
        ...
        Event.listen(this.numberCloudExtension, 'mouseleave', function (event) {this.requiresMouseLeaveEvent = false;}.bind(this));
    }
    ...
    Arbiter.subscribe('platform/socialplugins/dialog', function (l, m) {if (m.controllerID !== this.widgetID)this.otherCommentWidgetOpen = (m.event == 'open');}.bind(this));
    Arbiter.subscribe('platform/socialplugins/dialog', function (l, m) {if (m.controllerID !== this.divID && m.event == 'open')this.closeCommentWidget();}.bind(this));
...
copyProperties(ExternalNodeConnectWidget, {STANDARD_LAYOUT: 'standard', BOX_COUNT_LAYOUT: 'box_count', BUTTON_COUNT_LAYOUT: 'button_count', SIMPLE_LAYOUT: 'simple', COLOR_DARK: 0, COLOR_LIGHT: 1, OPT_IN_FACEBOOK_APP_ID: '127760087237610', updateLoggedInUser: function (a) {
    ...
    setTimeout(function () {c.positionNub(b);}, 1000);
}});
copyProperties(ExternalNodeConnectWidget.prototype, {getSyncEndpointName: function () {}, getConnectionEndpointName: function () {}, getPluginName: function () {}, subscribeToSync: function (a) {
    Arbiter.subscribe('platform/socialplugins/login', function (b, c) {if (this.viewerID && c.user !== this.viewerID)document.location.reload();}.bind(this));
...
}, placeDtsg: function (a) {Env.fb_dtsg = a;}, syncWidgetWithServer: function (a) {
    var b = a.getPayload();
    if (b.requires_login) {
...
}, conditionallyActivateButton: function (event) {this.commentButtonIsActive = this.inlineCommentInput.value.trim().length > 0;}, updateConnectionWithComment: function (event) {
    Event.kill(event);
...
}, postInlineComment: function () {this.postUserComment(this.inlineCommentInput.value.trim(), false, 'iframe');}, resetInlineComment: function () {
    ...
    if (this.layout === ExternalNodeConnectWidget.BOX_COUNT_LAYOUT) {this.manageBoxCountButtonAnimation(a);}
    else if (this.layout === ExternalNodeConnectWidget.BUTTON_COUNT_LAYOUT) {this.manageButtonCountButtonAnimation(a);}
    else if (this.layout === ExternalNodeConnectWidget.SIMPLE_LAYOUT) {this.manageSimpleButtonAnimation(a);}
    else this.manageStandardButtonAnimation(a);
    if (!this.loggedInUserPhotoDiv)return;
    if (a) {this.slideUserPhotoIn(this.loggedInUserPhotoDiv);}
    else this.slideUserPhotoOut(this.loggedInUserPhotoDiv);
}, manageStandardButtonAnimation: function (a) {
    if (this.grayOut)return;
    if (this.alreadyConnected || this.showConfirm) {this.hideButton(true);}
    ...
    if (!this._clickLocked) {
        ...
        if (this.viewerID == 0 || this.userOptedOut) {this.presentOptIn();}
    ...
    if (!a) {
        ...
        if (this.commentWidgetOpen) {this.closeCommentWidget();}
    ...
    if (a) {
        ...
        if (this.shareCommentSpans) {for (var d = 0; d < this.shareCommentSpans; d++)CSS.removeClass(this.shareCommentSpans[d], 'hidden_elem');}
    ...
    animation(this.userActionTextSpan).to('opacity', 0).duration(500).ondone(function () {
        ...
        CSS.removeClass(this.summaryTextSpan, 'hidden_elem');
        animation(this.summaryTextSpan).to('opacity', 1).duration(500).ondone(function () {this.repositionFacepileVertically();}.bind(this)).go();
    ...
    if (a && !this.showConfirm) {this.animateBoxCountButtonDisappearance();}
    ...
    CSS.addClass(this.numberCloudUnlikeSpan, 'hidden_elem');
    CSS.removeClass(this.numberCloudReminderSpan, 'hidden_elem');
    animation(this.numberCloud).to('height', this.expandedNumberCloudHeight).duration(100).ondone(function () {animation(this.numberCloudExtension).from('opacity', 0).to('opacity', 1).duration(100).go();}.bind(this)).go();
    ...
    animation(this.numberCloud).to('height', this.collapsedNumberCloudHeight).duration(75).ondone(function () {
        if (this.showConfirm) {for (var a = 0; a < this.confirmSpans.length; a++)CSS.removeClass(this.confirmSpans[a], 'hidden_elem');}
        else {
    ...
    if (a && !this.showConfirm) {this.animateButtonCountButtonDisappearance();}
    ...
    if (this.grayOut)return;
    if (a && !this.showConfirm) {this.animateSimpleButtonDisappearance();}
    else this.animateSimpleButtonReappearance();
}, animateButtonCountButtonDisappearance: function () {
    if (!this.grayOut)animation(this.connectButton).from('opacity', 1).to('opacity', 0).from('width', this.originalButtonWidth).to('width', '0px').ondone(function () {CSS.addClass(this.connectButton, 'hidden_elem');}.bind(this)).duration(100).go();
    ...
    Style.set(this.summaryIncluding, 'opacity', 1);
    animation(this.summaryIncluding).from('opacity', 1).to('opacity', 0).duration(100).ondone(function () {CSS.addClass(this.summaryIncluding, 'hidden_elem');}.bind(this)).go();
    ...
    if (this.layout === ExternalNodeConnectWidget.BOX_COUNT_LAYOUT || this.layout === ExternalNodeConnectWidget.BUTTON_COUNT_LAYOUT) {return this.widget;}
    else if (this.layout === ExternalNodeConnectWidget.SIMPLE_LAYOUT) {return this.widget;}
    else if (this.grayOut) {return DOM.find(this.widget, 'table.connect_widget_interactive_area');}
    ...
    return false;
}, shouldHideExpandedWidget: function () {return !this.commentWidgetIsExpanded || this.fullInputTextarea.value === '' || this.fullInputTextarea.value === this.defaultInputText;}, killCloseThread: function () {
    ...
    for (f = 0; f < b.length; f++) {
        ...
        e.appendChild(h);
        if (h.styleSheet) {h.styleSheet.cssText = i;}
        else {
    ...
    return false;
}, getAjaxEndpoint: function () {return '/ajax/connect/external_node_connect.php';}, getCommentAjaxEndpoint: function () {return '/ajax/connect/external_edge_comment.php';}, getConfirmURL: function () {
...
}, updateLoggedInUser: function (a, b) {ExternalNodeConnectWidget.updateLoggedInUser(a, b);}, syncSendWidgetWidth: function (a, b) {
...
ConnectWidget.prototype.disconnect = function () {
    if (!this.busy) {
        this.busy = true;
        fbpage_set_fan_status(this.button, this.profile_id, false, false, this.connectUndoneHandler.bind(this), function () {}, this.getAsyncData());
    }
};
...
ConnectWidget.prototype.connectErrorHandler = function (a) {
    if (a.error == 1357001) {
        ...
        CSS.hide(this.button);
        if (a.error == 1431001) {this.status = DOM.create('span', {className: 'connect_span'}, "You like this.");}
...
ConnectWidget.prototype.showNewFan = function () {
    ...
    this.fan_hidden.forEach(function (a) {CSS.hide(a);});
};
...
ConnectWidget.prototype.hideNewFan = function () {
    this.fan_shown.forEach(function (a) {
        ...
        animation(a).to('opacity', 0).duration(100).checkpoint(.25).to('width', 0).duration(100).ondone(function () {
            Style.set(a, 'width', null);
            CSS.hide(a);
            this.fan_hidden.forEach(function (b) {CSS.show(b);});
        }.bind(this)).go();
    }.bind(this));
};
ConnectWidget.loadStream = function (a, b, c) {new AsyncRequest().setMethod('GET').setURI('/ajax/connect/connect_widget.php').setData({id: a, uniqid: b, force_wall: c}).setReadOnly(true).setErrorHandler(bagofholding).send();};
...
ConnectWidget.setIsConnect = function (a) {ConnectWidget.is_connect = a;};
...
}, getConnectionEndpointName: function () {return 'platform/like/connection';}, getPluginName: function () {return 'like';}, presentInsightsPage: function (event) {
    Event.kill(event);
    var a;
    a = '/insights/?inline_link=true&sk=';
    if (this.pageId) {a += 'po_' + this.pageId;}
