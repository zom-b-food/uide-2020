if (!window.__DBW)var __DBW = function () {
    var h = {api: "http://widgets.digg.com/buttons/count?url=", analytics: "http://a.digg.com/", css: "http://widgets.digg.com/css/buttons3.css", popup: "http://digg.com/tools/diggthis/", submit: "http://digg.com/submit?"}, z = {impression: 5, click: 100}, s = {source: "source", submitted: "submitted", event: "event", diggs: "diggs", storyId: "storyId", buttons: "buttons"}, i, j, l = {}, A = function (a, b) {
            ...
            function b() {for (var f in e)return f}
            ...
            c.firstChild.onclick = function () {
                ...
                if (!k) {
                    ...
                    j.ie ? t(f) : u(f, 0, function () {
                        t(f);
                        setTimeout(function () {u(f, 100)}, 175)
                    });
                    k = 1
                }
                b() ? n(e, "click") : setTimeout(function () {n(e, "click")}, 1E3);
                D(g, e)
            };
        ...
        }, E = function (a) {F(h.api + a)}, B = function (a) {
            var b = "large", c = " " + a.className + " ", d = "";
            if (c.match(/( DiggThisButtonMedium | DiggMedium )/))b = "medium";
            else if (c.match(/ DiggLarge /))b = "large";
            else if (c.match(/ DiggCompact /))b = "compact";
            else if (c.match(/ DiggIcon /))b = "digger";
            else if (c.match(/ DiggWide /))b = "wide";
            a = a.getElementsByTagName("IMG");
            if (a[0])d = a[0].src;
            else return b;
            if (d.match(/diggThis\.(gif|png)/i))return"large";
            else if (d.match(/diggThisMedium\.(gif|png)/i))return"medium";
            else if (d.match(/diggThisCompact\.(gif|png)/i))return"compact";
            else if (d.match(/digg-guy-icon.(gif|png)/i))return"digger";
            else if (d.match(/diggThisWide.(gif|png)/i))return"wide";
            else if (d.match(/diggThisIcon.(gif|png)/i))return"digger";
            return b
        }, t = function (a) {
        ...
        }, H = function (a) {return a.replace(/^\s+|\s+$/g, "")}, q, x, p = 0, y, r = function () {
            o && o();
            i || I();
            y();
            x || setTimeout(r, 500)
    ...
    y = function (a) {return a && (a += "").substr(a.indexOf("{")).replace(/\s/g, "") == "{[nativecode]}"}(document.getElementsByClassName) ? function () {
    ...
    return{writeLink: function () {
        ...
        if (window.digg_skin == "compact")a.push(' DiggCompact"');
        else window.digg_skin == "icon" ? a.push(' DiggIcon"') : a.push('"');
        if (window.digg_title)a.push(' href="', h.submit, "url=", escape(digg_url || DIGG_URL || location.href).replace(/\+/g, "%2b"), "&title=", escape(digg_title), '"');
        else if (window.digg_url || window.DIGG_URL)a.push(' href="', h.submit, "url=", escape(digg_url || DIGG_URL).replace(/\+/g,
            "%2b"), '"');
    ...
    }, addButtons: r, collectDiggs: function (a) {for (var b = l[escape(a.url).replace(/\+/g, "%2b")], c = b.length, d = 0; d < c; d++)b[d](a)}}
}();