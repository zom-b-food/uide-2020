var _bsap = {apiurl: 'http:\/\/s3.buysellads.com\/r\/', dz: [], dii: [], inv: [], drop: function (a, b) {
    ...
    bsa.filter(z.filterby, function (f) {
        ...
        if (t <= 1) {var ah = (typeof(ShowAdHereBanner) === 'object' ? ShowAdHereBanner[z.id] : z.showadhere) > 0, ra = (typeof(RepeatAll) === 'object' ? RepeatAll[z.id] : z.repeathere) > 0}
        if (tf == 0) {
            ...
            if (a && (Math.random() * 100 > (z.model == 1 && z.nads > c.length ? (100 - c[0].per) : empty))) {
                ...
                b.onclick = function (e) {if (e.target.getAttribute('id') == ('bsap_' + z.id))window.location = bsa.tracker('click.go', a.id, z.id, '', '', '&link=' + a.link.replace('[timestamp]', ts))};
                t += a.id
            }
        }
    ...
    while (ret.length < n && i++ < n * 100 && (c = a[b[Math.floor(Math.random() * b.length)]]))if (n === 1)return[c];
    else if (!got[c.id]) {
        got[c.id] = 1;
        ret.push(c)
    }
...
}, link: function (b, z, l) {return'href="' + this.tracker('click.go', b, z, '', '', '&link=' + l) + '" onmouseover="window.status = \'' + l + '\'; return true;" onmouseout="window.status=\'\'; return true;"'}, tracker: function (t, b, z, u, s, additional) {return'http:\/\/stats.buysellads.com\/' + t + '?z=' + z + '&b=' + b + '&g=' + u + '&s=' + s + '&sw=' + screen.width + '&sh=' + screen.height + '&br=' + this.br() + '&r=' + Math.random() + additional}, br: function () {
    ...
    catch (e) {if (navigator.mimeTypes["application/x-shockwave-flash"] != undefined)gotit = true}
    return gotit
}};
_bsap_loadedme = 1;
...
if (document.addEventListener)document.addEventListener('DOMContentLoaded', function () {_bsap.exec()}, false);
