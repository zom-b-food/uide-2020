if (!__qc) {
    var __qc = {qcdst: function () {
        ...
        var qh = __qc.qhash(d);
        for (var i = 0; i < __qc.sd.length; i++) {if (__qc.sd[i] == qh)return";fpan=u;fpa=";}
        var u = document;
        ...
        if (myqo != null) {
            for (var k in myqo) {
                ...
                if (k == 'uh') {
                    if (__qc.qcdnt()) {delete myqo[k];}
                    else {myqo[k] = __qc.qhash(myqo[k]);}
                }
                if (k == 'qacct') {
                    a = myqo[k];
                    continue;
        ...
        var qocount = 0;
        __qc.qad = 0;
        if (typeof __qc.qpixelsent == "undefined") {__qc.qpixelsent = new Array();}
        ...
        else if (typeof _qacct == "string") {qo = __qc.qcp("", null);}
        ...
        var dst = __qc.qcdst();
        var qs = 'http';
        if (window.location.protocol == 'https:') {qs += 's';}
        ...
        img.onload = function () {__qc.qpxload(img);}
    }, quantserve: function () {
        ...
        else if (!_qevents.length && typeof _qacct != "undefined") {__qc.firepixel(null);}
        if (!__qc.evts) {
            for (var k in _qevents) {__qc.firepixel(_qevents[k]);}
            _qevents = {push: function () {
                var a = arguments;
                for (var i = 0; i < a.length; i++) {__qc.firepixel(a[i]);}
            }};
            __qc.evts = 1;
        }
    }};
