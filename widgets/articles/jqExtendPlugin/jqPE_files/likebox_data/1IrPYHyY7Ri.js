...
__d("CSSSupport",[],function(a,b,c,d,e,f){var g={supportsBorderRadius:function(){var h=['KhtmlBorderRadius','OBorderRadius','MozBorderRadius','WebkitBorderRadius','msBorderRadius','borderRadius'],i=false,j=document.createElement('div');for(var k=h.length;k>=0;k--)if(i=j.style[h[k]]!==undefined)break;g.supportsBorderRadius=bagof(i);return i;},supportsPositionSticky:function(){var h='position',i=['-webkit-sticky','-moz-sticky','-o-sticky','-ms-sticky','sticky'],j=document.createElement('div'),k='';i.forEach(function(m){k+=h+':'+m+';';});j.style.cssText=k;var l=(/sticky/).test(j.style[h]);g.supportsPositionSticky=bagof(l);return l;}};e.exports=g;});
...
__d("requestAnimationFrame",[],function(a,b,c,d,e,f){var g=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(h){window.setTimeout(h,33);};e.exports=g;});
__d("OnVisible",["event-extensions","Arbiter","DOM","Run","Vector","copyProperties","requestAnimationFrame"],function(a,b,c,d,e,f){b('event-extensions');var g=b('Arbiter'),h=b('DOM'),i=b('Run'),j=b('Vector'),k=b('copyProperties'),l=b('requestAnimationFrame'),m=[],n,o=[],p,q,r,s;function t(){m.forEach(function(z){z.remove();});w=false;if(q){q.remove();p.remove();n.unsubscribe();q=p=n=null;}m.length=0;}function u(){if(!m.length){t();return;}var z,aa;o.length=0;r=j.getScrollPosition().y;s=j.getViewportDimensions().y;for(z=0;z<m.length;++z){aa=m[z];if(isNaN(aa.elementHeight))o.push(z);aa.elementHeight=j.getElementDimensions(aa.element).y;aa.elementPos=j.getElementPosition(aa.element);}for(z=m.length-1;z>=0;--z){aa=m[z];if(!aa.elementPos||aa.removed){m.splice(z,1);continue;}var ba=r+s+aa.buffer;if(ba>aa.elementPos.y){var ca=!aa.strict||(r-aa.buffer<(aa.elementPos.y+aa.elementHeight));if(ca){aa.remove();aa.handler(o.indexOf(z)!==-1);}}}w=false;}function v(){x();if(m.length)return;q=Event.listen(window,'scroll',x);p=Event.listen(window,'resize',x);n=g.subscribe('dom-scroll',x);}var w=false;function x(){if(w)return false;w=true;l(l.curry(u));}function y(z,aa,ba,ca){this.element=z;this.handler=aa;this.strict=ba;this.buffer=ca||300;if(m.length===0)i.onLeave(t);v();m.push(this);}k(y.prototype,{remove:function(){this.removed=true;},reset:function(){this.elementHeight=null;m.indexOf(this)===-1&&m.push(this);v();},setBuffer:function(z){this.buffer=z;}});e.exports=y;});
...
__d("UserActivity",["event-extensions","Arbiter"],function(a,b,c,d,e,f){b('event-extensions');var g=b('Arbiter'),h=5000,i=500,j=-5,k=Date.now(),l=k,m={subscribeOnce:function(o){var p=m.subscribe(function(){m.unsubscribe(p);o();});},subscribe:function(o){return g.subscribe('useractivity/activity',o);},unsubscribe:function(o){o.unsubscribe();},isActive:function(o){return (new Date()-k<(o||h));},getLastInformTime:function(){return l;}};function n(event){k=Date.now();var o=k-l;if(o>i){l=k;g.inform('useractivity/activity',{event:event,idleness:o,last_inform:l});}else if(o<j)l=k;}Event.listen(window,'scroll',n);Event.listen(document.documentElement,{DOMMouseScroll:n,mousewheel:n,keydown:n,mouseover:n,click:n});e.exports=m;});
...
__d("TimelineController",["event-extensions","Arbiter","CSS","DataStore","DOM","Run","ScrollingPager","Style","TimelineConstants","TimelineURI","Vector","ViewportBounds","$","ge","throttle"],function(a,b,c,d,e,f){b('event-extensions');var g=b('Arbiter'),h=b('CSS'),i=b('DataStore'),j=b('DOM'),k=b('Run'),l=b('ScrollingPager'),m=b('Style'),n=b('TimelineConstants'),o=b('TimelineURI'),p=b('Vector'),q=b('ViewportBounds'),r=b('$'),s=b('ge'),t=b('throttle'),u=484,v=48,w=740,x=1285,y=null,z=false,aa,ba,ca,da,ea={},fa={},ga=[],ha=null,ia=null,ja=0,ka=false,la=false,ma=false,na={allactivity:true,approve:true,games:true,map:true,music:true,video:true};na[o.TIMELINE_KEY]=true;var oa=[],pa=false;function qa(){ha&&ha.remove();ha=null;}function ra(ab,bb,cb){cb=cb||[];if(ea[ab])return ea[ab][bb].apply(ea[ab],cb);fa[ab]=fa[ab]||{};fa[ab][bb]=cb;return false;}function sa(){if(!(ka||ma||la)){qa();return;}var ab=p.getScrollPosition();ka=ka&&ua(s('rightCol'),ab,'paddingTop',true);la=la&&ua(r('pagelet_above_header_timeline'),ab,'top');ma=ma&&ua(r('blueBar'),ab,'paddingTop');}function ta(){var ab=p.getScrollPosition(),bb=ja===0||ab.y>=ja;ra(za.STICKY_HEADER,'toggle',[bb]);ra(za.CONTENT,'checkCurrentSectionChange');}function ua(ab,bb,cb,db){if(!ab){qa();return;}if(bb.y<=0){va(ab,cb);return false;}else{var eb=db&&za.getCurrentScrubber();if((eb&&h.hasClass(eb.getRoot(),'fixed_elem'))||m.isFixed(s('pagelet_timeline_friends'))){va(ab,cb);return false;}else{var fb=parseInt(ab.style[cb],10)||0;if(bb.y<fb){h.addClass(ab,'timeline_fixed');ab.style[cb]=bb.y+'px';}else h.removeClass(ab,'timeline_fixed');}}return true;}function va(ab,bb){ab.style[bb]='0px';h.removeClass(ab,'timeline_fixed');}function wa(){ra(za.ADS,'adjustAdsType',[za.shouldShowWideAds()]);ra(za.ADS,'adjustAdsToFit');ra(za.CONTENT,'adjustContentPadding');ra(za.STICKY_HEADER_NAV,'adjustMenuHeights');}function xa(ab,bb){if(ab=='sidebar/initialized')pa=true;ra(za.ADS,'adjustAdsType',[za.shouldShowWideAds()]);}function ya(){while(ga.length)ga.pop().remove();for(var ab in ea)ea[ab].reset&&ea[ab].reset();qa();da.unsubscribe();da=null;y=null;aa=null;ea={};fa={};ia=null;ja=0;la=false;if(ka){var bb=s('rightCol');if(bb){bb.style.paddingTop='';h.removeClass(bb,'timeline_fixed');}}ka=false;if(ma){r('blueBar').style.paddingTop='';h.removeClass(r('blueBar'),'timeline_fixed');}ma=false;oa=[];pa=false;z=false;i.purge(n.DS_HEIGHT);i.purge(n.DS_LOADED);i.purge(n.DS_SIDEORG);i.purge(n.DS_TAILBALANCE);i.purge(n.DS_COLUMN_HEIGHT_DIFFERENTIAL);}var za={NAV:'nav',STICKY_HEADER:'sticky_header',STICKY_HEADER_NAV:'sticky_header_nav',SCRUBBER:'scrubber',CONTENT:'content',ADS:'ads',COVER:'cover',LOGGING:'logging',SECTION_LOADING:'TimelineController/sectionLoading',SECTION_LOADED:'TimelineController/sectionLoaded',SECTION_FULLY_LOADED:'TimelineController/sectionFullyLoaded',SECTION_REGISTERED:'TimelineController/sectionRegistered',SCRUBBER_DEFAULT_OFFSET:38,init:function(ab,bb,cb){if(z)return;if(bb==o.WALL_KEY)bb=o.TIMELINE_KEY;z=true;aa=ab;ba=cb.has_fixed_ads;ca=cb.force_wide_ads;ra(za.CONTENT,'adjustContentPadding');ga.push(Event.listen(window,'scroll',t(ta,100)),Event.listen(window,'resize',t(wa,100)));da=g.subscribe(['sidebar/initialized','sidebar/show','sidebar/hide'],xa);k.onLeave(ya);za.registerCurrentKey(bb);},setAdsTracking:function(ab){ra(za.ADS,'start',[ab]);},coverHasExpanded:function(ab){var bb=s('rightCol');if(bb){bb.style.paddingTop=ab+'px';ka=true;}var cb=r('pagelet_above_header_timeline');if(cb.firstChild){r('above_header_timeline_placeholder').style.height=cb.offsetHeight+'px';cb.style.top=ab+'px';la=true;}var db=document.documentElement;ma=db.clientHeight<400||db.clientWidth<db.scrollWidth;if(ma)r('blueBar').style.paddingTop=ab+'px';ha=Event.listen(window,'scroll',sa);g.inform('reflow');},pageHasScrubber:function(ab){return !ab||ab.match(/^(og_)?app_/)||(ab in na);},fixedAds:function(){return ba;},registerCurrentKey:function(ab){y=ab;ia=ab!=='map'&&p.getViewportDimensions().y<w&&za.pageHasScrubber(ab);ra(za.ADS,'setShortMode',[ia]);ra(za.ADS,'updateCurrentKey',[ab]);ja=ab==o.TIMELINE_KEY?u-v:0;},getCurrentKey:function(){return y;},getSectionKeys:function(){return oa;},getCurrentScrubber:function(){return ea[za.SCRUBBER];},getCurrentStickyHeaderNav:function(){return ea[za.STICKY_HEADER_NAV];},getSection:function(ab){return ra(za.CONTENT,'getSection',[ab]);},scrubberHasLoaded:function(ab){ra(za.STICKY_HEADER_NAV,'addTimePeriods',[oa]);h.conditionClass(ab.getRoot(),'fixed_elem',!ia);ra(za.ADS,'registerScrubber',[ab]);},registerSectionKey:function(ab,bb){if(ea[za.STICKY_HEADER_NAV]){ra(za.STICKY_HEADER_NAV,'addTimePeriod',[ab]);}else oa[bb]=ab;},scrubberHasChangedState:function(){ra(za.ADS,'adjustAdsToFit');},scrubberWasClicked:function(ab){ra(za.LOGGING,'logScrubberClick',[ab]);},sectionHasChanged:function(ab,bb){ra(za.STICKY_HEADER_NAV,'updateSection',[ab,bb]);ra(za.SCRUBBER,'updateSection',[ab,bb]);ra(za.ADS,'loadAdsIfEnoughTimePassed');ra(za.LOGGING,'logSectionChange',[ab,bb]);},navigateToSection:function(ab){ra(za.CONTENT,'navigateToSection',[ab]);},shouldShowWideAds:function(){if(ca)return true;if(!pa)return false;var ab=x+q.getRight();return p.getViewportDimensions().x>=ab;},sidebarInitialized:function(){return pa;},adjustStickyHeaderWidth:function(){ra(za.STICKY_HEADER,'adjustWidth');},register:function(ab,bb){ea[ab]=bb;if(fa[ab]){for(var cb in fa[ab])ra(ab,cb,fa[ab][cb]);delete fa[ab];}},adjustScrollingPagerBuffer:function(ab,bb){var cb=i.get(n.DS_COLUMN_HEIGHT_DIFFERENTIAL,bb);if(!cb)return;var db=l.getInstance(ab);db&&db.setBuffer(db.getBuffer()+Math.abs(cb));},runOnceWhenSectionFullyLoaded:function(ab,bb,cb){if(ra(za.CONTENT,'getSection',[bb])){var db=false;j.scry(ea[za.CONTENT].getSection(bb).node,'ol.fbTimelineCapsule').forEach(function(fb){if(!db&&parseInt(i.get(n.DS_LOADED,fb.id),10)>=parseInt(cb,10)){ab();db=true;}});if(db)return;}var eb=g.subscribe(za.SECTION_FULLY_LOADED,function(fb,gb){if(gb.scrubberKey===bb&&gb.pageIndex===cb){ab();eb.unsubscribe();}});}};e.exports=za;});
