...
------------------------------------------------------------------------------------------------------------------*/
         ...
         * @since  1.1.8.1
         *
         * @return bool TRUE if successfuly turned on.
         */
                        ...
                        $message = sprintf(
                            ...
                            sprintf(
                                ...
                                sprintf(
                                    '<b>%s</b> - %s',
                                    $sysadmin_title,
                                    esc_html( sprintf( $this->get_text_inline( 'Great, please install cURL and enable it in your php.ini file. In addition, search for the \'disable_functions\' directive in your php.ini file and remove any disabled methods starting with \'curl_\'. To make sure it was successfuly activated, use \'phpinfo()\'. Once activated, deactivate the %s and reactivate it back again.', 'curl-missing-sysadmin-desc' ), $this->get_module_label( true ) ) )
                                ),
                                sprintf(
                                    '<a href="%s"><b>%s</b></a> - %s',
...
//				}
         ...
         *  2. TRUE   - If successfuly opted-out.
         ...
         * Returns:
         *  1. FALSE  - If the user never opted-in.
         *  2. TRUE   - If successfuly opted-in back to usage tracking.
         *  3. object - API result on failure.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.2.1.5
        ...
        function _plugin_code_type_changed() {
            ...
            if ( $this->is_premium() ) {
                ...
                if ( ! $this->is_only_premium() ) {
                    $notice = sprintf( $this->get_text_inline( 'Premium %s version was successfuly activated.', 'premium-activated-message' ), $this->_module_type );
                }
                        ...
                        sprintf(
                            ( $is_after_trial_cancel ?
                                $this->_parent->get_text_inline(
                                    '%s free trial was successfuly cancelled. Since the add-on is premium only it was automatically deactivated. If you like to use it in the future, you\'ll have to purchase a license.',
...
//				$this->sync_install( array(), true );
                ...
                if ( $is_premium_version_activation ) {
                    $this->_admin_notices->add(
                        sprintf( $this->get_text_inline( 'The upgrade of %s was successfuly completed.', 'successful-version-upgrade-message' ), sprintf( '<b>%s</b>', $this->_plugin->title ) ),
                        $this->get_text_x_inline( 'W00t',
                            'Used to express elation, enthusiasm, or triumph (especially in electronic communication).', 'woot' ) . '!'
                    );
                }
            ...
            if ( $this->is_premium() &&
                ...
                if ( is_object( $this->_site ) ) {
                    if ( $this->_site->license_id == $license->id ) {
                        /**
                         * If the license was activated successfuly, sync the license data from the remote server.
                         */
...
//            }
        ...
        private function send_install_update( $override = array(), $flush = false ) {
            ...
            if ( ! $keepalive_only_update && $this->is_api_result_entity( $site ) ) {
                 ...
                 */
                if ( ! is_multisite() ) {
                    // I successfuly sent install update, clear scheduled sync if exist.
                    $this->clear_install_sync_cron();
                }
            }
        ...
        private function send_installs_update( $override = array(), $flush = false ) {
            ...
            if ( ! $keepalive_only_update && $this->is_api_result_object( $result, 'installs' ) ) {
                // I successfuly sent installs update (there was an actual update sent and it's not just a keepalive-only call), clear scheduled sync if exist.
                $this->clear_install_sync_cron();
            }
...
//            $is_network_level_opt_in = self::is_ajax_action_static( 'network_activate', $this->_module_id );
            ...
            if ( $this->is_pending_activation() || ! $this->has_settings_menu() ) {
                ...
                if ( ! $this->is_paying_or_trial() ) {
                    $this->_admin_notices->add_sticky(
                        sprintf( $this->get_text_inline( '%s activation was successfuly completed.', 'plugin-x-activation-message' ), '<b>' . $this->get_plugin_name() . '</b>' ),
                        'activation_complete'
                    );
                }
            }
            ...
            if ( $this->is_paying_or_trial() ) {
                if ( ! $this->is_premium() ||
                    ...
                    if ( $this->is_paying() ) {
                        $this->_admin_notices->add_sticky(
                            sprintf(
                                $this->get_text_inline( 'Your account was successfuly activated with the %s plan.', 'activation-with-plan-x-message' ),
                                $this->get_plan_title()
                            ) . $this->get_complete_upgrade_instructions(),
                            'plan_upgraded',
                            $this->get_text_x_inline( 'Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw' ) . '!'
                        ...
                        $this->_admin_notices->add_sticky(
                            sprintf(
                                $this->get_text_inline( 'Your trial has been successfuly started.', 'trial-started-message' ),
...
//			}
        ...
        private function _handle_account_user_sync() {
            ...
            if ( isset( $user->id ) ) {
                ...
                if ( $user->is_verified &&
                    ...
                    $this->_admin_notices->add(
                        $this->get_text_inline( 'Your email has been successfuly verified - you are AWESOME!', 'email-verified-message' ),
        ...
        function _set_data_debug_mode() {
            ...
            if ( 'true' === $transient_value ) {
                $this->_admin_notices->add_sticky(
                    $this->get_text_inline(
                        'Debug mode was successfuly enabled and will be automatically disabled in 60 min. You can also disable it earlier by clicking the "Stop Debug" link.',
                        ...
                        $this->_admin_notices->add_sticky(
                            sprintf(
                                ( FS_Plan_Manager::instance()->has_free_plan( $plans ) ?
                                    $this->get_text_inline( 'Your %s Add-on plan was successfuly upgraded.', 'addon-successfuly-upgraded-message' ) :
                                    /* translators: %s:product name, e.g. Facebook add-on was successfuly... */
                                    $this->get_text_inline( '%s Add-on was successfuly purchased.', 'addon-successfuly-purchased-message' ) ),
                                $addon->title
            ...
            if ( $this->apply_filters( 'has_paid_plan_account', $this->has_paid_plan() ) ) {
                switch ( $plan_change ) {
                    ...
                    case 'activated':
                        $this->_admin_notices->add_sticky(
                            ( 'activated' === $plan_change ) ?
                                $this->get_text_inline( 'Your plan was successfuly activated.', 'plan-activated-message' ) :
                                $this->get_text_inline( 'Your plan was successfuly upgraded.', 'plan-upgraded-message' ) .
                            $this->get_complete_upgrade_instructions(),
                            'plan_upgraded',
                            $this->get_text_x_inline( 'Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw' ) . '!'
                        );
                    ...
                    case 'changed':
                        $this->_admin_notices->add_sticky(
                            sprintf(
                                $this->get_text_inline( 'Your plan was successfuly changed to %s.', 'plan-changed-to-x-message' ),
                                $this->get_plan_title()
                            ),
                            'plan_changed'
                        );
        ...
        protected function _activate_license( $background = false, $premium_license = null ) {
            ...
            if ( ! $background ) {
                $this->_admin_notices->add_sticky(
                    $this->get_text_inline( 'Your license was successfuly activated.', 'license-activated-message' ) .
                    $this->get_complete_upgrade_instructions(),
                    'license_activated',
                    $this->get_text_x_inline( 'Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw' ) . '!'
                );
        ...
        private function handle_license_deactivation_result( $license, $hmm_text = false, $show_notice = true ) {
            ...
            if ( $show_notice ) {
                $this->_admin_notices->add(
                    sprintf( $this->get_text_inline( 'Your license was successfuly deactivated, you are back to the %s plan.', 'license-deactivation-message' ), $this->get_plan_title() ),
        ...
        private function _downgrade_site() {
            ...
            $this->_admin_notices->add(
                sprintf( $this->get_text_inline( 'Your subscription was successfuly cancelled. Your %s plan license will expire in %s.', 'plan-x-downgraded-message' ),
                    $plan->title,
         ...
         *
         * @return bool If trial was successfuly started.
         */
        ...
        private function _cancel_trial() {
            ...
            if ( ! $this->is_addon() ||
                ...
                $this->_admin_notices->add(
                    sprintf( $this->get_text_inline( 'Your %s free trial was successfuly cancelled.', 'trial-cancel-message' ), $trial_plan->title )
                );
            }
         ...
         *
         * @return bool Is ownership change successfuly initiated.
         */
         ...
         * @uses   FS_Api
         *
         * @return bool Was ownership change successfuly complete.
         */
            ...
            switch ( $action ) {
                    ...
                    if ( $plugin_id == $this->get_id() ) {
                        if ( $is_network_action && ! empty( $blog_id ) ) {
                            if ( ! $this->is_registered() ) {
                                ...
                                $this->_admin_notices->add(
                                    $this->get_text_inline( 'Site successfuly opted in.', 'successful-opt-in' ),
                            ...
                            if ( $this->is_registered() ) {
                                if ( $this->is_tracking_prohibited() ) {
                                    ...
                                    if ( $this->stop_site_tracking() ) {
                                        $this->_admin_notices->add(
                                            sprintf(
                                                $this->get_text_inline( 'We will no longer be sending any usage data of %s on %s to %s.', 'opted-out-successfuly' ),
                ...
                case 'update_email':
                    ...
                    if ( isset( $result->error ) ) {
                        ...
                        $this->_admin_notices->add( $this->get_text_inline( 'Your email was successfuly updated. You should receive an email with confirmation instructions in few moments.', 'email-updated-message' ) );
                    }
                ...
                case 'update_user_name':
                    ...
                    if ( isset( $result->error ) ) {
                        ...
                        $this->_admin_notices->add( $this->get_text_inline( 'Your name was successfuly updated.', 'name-updated-message' ) );
                    }
                ...
                foreach ( $properties as $p ) {
                    if ( 'update_' . $p === $action ) {
                        ...
                        /* translators: %s: User's account property (e.g. email address, name) */
                            $this->get_text_inline( 'You have successfuly updated your %s.', 'x-updated' ),
                            '<b>' . str_replace( '_', ' ', $p ) . '</b>'
                        ) );
